            ...
            <el-form ref="ruleFormRef" :model="ruleForm" :rules="rules" class="demo-ruleForm" status-icon @submit.native.prevent>
              ...
              <el-form-item prop="newEmail">
                <label class="label" for="newEmail">
                  <div class="flex flex-row">
                    <el-input v-model="ruleForm.newEmail" placeholder="New Email" />
                  </div>
                </label>
              </el-form-item>
              <el-form-item>
                <el-button type="primary" size="large" @click="submitForm('ruleFormRef')">Save</el-button>
              </el-form-item>
            </el-form>
          </el-col>
        </el-row>
  ...
  setup () {
    ...
    const validateNewEmail = (rule, value, callback) => {
      if (value !== ruleForm.email) {
        callback(new Error("Inconsistent email addresses"));
      } else {
        callback();
      }
    }
    ...
    const sendCode = async () => {
      if (regEmail.test(ruleForm.email)) timerHandler()
      else sms.tip = true
    }
    onActivated(async () => {
